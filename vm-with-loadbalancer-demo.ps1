...
$lb_data= az network lb address-pool list --lb-name $LB_Name 
$conv= $lb_data | ConvertFrom-Json 
$ap = $conv.name 
   

if ($backendpool -ne $null) {$webserver="nginx"} else {$webserver="apache2"}
...
for ($i = 0; $i -lt $backendpool.Count; $i++) { echo $backendpool[$i] }
...
#Install Webserver
echo "Install Webserver: $webserver"
...
foreach ($ips in $backendpool)
{
    $counter++
    $cmd+=" --backend-address name=addr$counter ip-address=$ips"
}
